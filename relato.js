...
const RELATO = {
  ...
  /* ---- CONFERÊNCIAS DE IMPRENSA ----
     fx: morale (equipa) · board (confiança direção) · fans (apoio dos adeptos) · rep (reputação) ·
         rival (humor do rival, +provoca) · pmorale (jogador mencionado)
     target (em perguntas sobre {jogador}): "star" (jogador em destaque) · "bench" (pouco utilizado) · ausente=aleatório
     Categorias: pre · post (com cond: win/bigwin/loss/bigloss/draw/any) · tatica · banco · misc */
  imprensa: {
    pre: [
      { q:"Vão ao jogo com o {adv} como favoritos?",
        opts:[ {label:"Somos favoritos e vamos ganhar", fx:{morale:2, board:1, rival:1, fans:3}},
               {label:"Respeitamos o {adv}, vai ser difícil", fx:{board:1, fans:-1}},
               {label:"Não faço previsões", fx:{}} ] },
      { q:"Que dizes sobre o momento de {jogador}?", target:"star",
        opts:[ {label:"Confio totalmente nele", fx:{pmorale:4, fans:2}},
               {label:"Tem de dar mais", fx:{pmorale:-4, board:1, fans:-1}},
               {label:"Não falo de jogadores em particular", fx:{}} ] },
      { q:"Provocam-te sobre o {rival}. Respondes?",
        opts:[ {label:"Vamos passar-lhes por cima", fx:{morale:3, rival:3, board:-1, fans:5}},
               {label:"Foco no nosso trabalho", fx:{}},
               {label:"Respeito o {rival}", fx:{rival:-2, board:1, fans:-3}} ] },
      ...
      { q:"Os adeptos esperam espetáculo. Prometes golos?",
        opts:[ {label:"Vamos dar-lhes uma festa!", fx:{fans:5, morale:2, board:-1}},
               {label:"Prometo entrega, o resto vem", fx:{fans:1}},
               {label:"O importante é ganhar, seja como for", fx:{fans:-2, board:1}} ] },
      { q:"Como está o balneário para este jogo?",
        opts:[ {label:"Nunca vi o grupo tão ligado", fx:{morale:3, fans:1}},
               {label:"Concentrados e humildes", fx:{morale:1}},
               {label:"Há muito a melhorar, e eles sabem", fx:{morale:-2, board:1}} ] },
      { q:"O {adv}, orientado por {tradv}, muda muito de jogo para jogo. Difícil de preparar?",
        opts:[ {label:"Estudámos tudo, estamos prontos", fx:{board:1}},
               {label:"Preocupa-me mais a minha equipa", fx:{}},
               {label:"Seja qual for o plano deles, ganhamos", fx:{fans:2, rival:1, board:-1}} ] },
      { q:"Falou-se de interesse de outro clube em ti. Confirmas?",
        opts:[ {label:"Estou muito feliz aqui", fx:{board:3, fans:2}},
               {label:"Nunca digo nunca", fx:{board:-3, fans:-2}, rumor:"o treinador do {clube} não fechou a porta a uma saída."},
               {label:"Não comento rumores", fx:{}} ] },
      { q:"Uma palavra para os adeptos que vão encher a bancada?",
        opts:[ {label:"São o nosso 12º jogador!", fx:{fans:6, morale:1}},
               {label:"Contamos com eles até ao fim", fx:{fans:3}},
               {label:"Que venham, mas o foco é o jogo", fx:{fans:-1}} ] }
    ],
    post: [
      { cond:"win", q:"Qual foi o segredo da vitória?",
        opts:[ {label:"Todo o mérito é dos jogadores", fx:{morale:3, fans:3}},
               {label:"O meu trabalho está a dar frutos", fx:{rep:2, morale:-1, fans:-1}},
               {label:"Também tivemos alguma sorte", fx:{board:-1, fans:1}} ] },
      { cond:"win", q:"Os adeptos saíram em euforia. Recado para eles?",
        opts:[ {label:"Esta vitória é deles!", fx:{fans:6}},
               {label:"Ainda não ganhámos nada", fx:{board:1, fans:-1}},
               {label:"Vamos por mais!", fx:{fans:2, morale:1}} ] },
      { cond:"bigwin", q:"Goleada! A equipa foi implacável hoje.",
        opts:[ {label:"Somos candidatos, digo-o já", fx:{fans:6, board:-2, rival:2}},
               {label:"Foi um bom dia, só isso", fx:{board:1}},
      ...
      { cond:"loss", q:"Ouviram-se assobios no final. Preocupa-te?",
        opts:[ {label:"Têm todo o direito, vamos corrigir", fx:{fans:3, board:1}},
               {label:"Compreendo a frustração deles", fx:{fans:1}},
               {label:"Não ligo a assobios", fx:{fans:-5, board:-1}} ] },
      { cond:"bigloss", q:"Uma derrota pesada. O que falhou?",
        opts:[ {label:"A culpa é minha, dou a cara", fx:{board:3, fans:2, morale:1}},
               {label:"Foi um dia para esquecer depressa", fx:{fans:-2}},
               {label:"Vamos treinar o dobro esta semana", fx:{morale:-2, board:1}} ] },
      { cond:"draw", q:"Empate. Dois pontos perdidos ou um ganho?",
        opts:[ {label:"Dois perdidos, queríamos mais", fx:{fans:2, board:-1}},
               {label:"Um ponto que se soma", fx:{board:1}},
               {label:"Depende do que aí vem", fx:{}} ] },
      { cond:"any", q:"Uma palavra sobre a exibição de {jogador}?", target:"star",
        opts:[ {label:"Esteve enorme, é um orgulho", fx:{pmorale:5, fans:2}},
               {label:"Pode e tem de fazer melhor", fx:{pmorale:-4, board:1, fans:-1}},
               {label:"Avalio a equipa como um todo", fx:{morale:1}} ] },
      { cond:"any", q:"O que muda daqui para a frente?",
        opts:[ {label:"Vamos lutar por tudo", fx:{fans:2, morale:1}},
               {label:"Um jogo de cada vez", fx:{}},
               {label:"Há muito a melhorar", fx:{morale:-1, board:1}} ] }
    ],
    // perguntas táticas
    tatica: [
      { q:"Voltaste a apostar no bloco baixo. É a tua identidade?",
        opts:[ {label:"É o que nos dá resultados", fx:{board:1}},
               {label:"Adaptamo-nos a cada jogo", fx:{rep:1}},
               {label:"Prefiro atacar, mas há que ter juízo", fx:{fans:-1}} ] },
      { q:"A equipa sofre muitos golos. É problema defensivo?",
        opts:[ {label:"Estamos a trabalhar nisso", fx:{}},
               {label:"Marcamos mais do que sofremos", fx:{fans:1, board:-1}},
               {label:"A responsabilidade é coletiva", fx:{morale:1}} ] },
      { q:"Muita gente pede um médio mais criativo no onze. Ouves?",
        opts:[ {label:"Quem decide o onze sou eu", fx:{board:1, fans:-2}},
               {label:"Tenho isso em conta", fx:{fans:1}},
               {label:"Talvez seja altura de mudar", fx:{fans:2, morale:-1}} ] },
      { q:"Jogas sempre no mesmo sistema. Não é previsível?",
        opts:[ {label:"Se funciona, não se mexe", fx:{board:1}},
               {label:"Temos variantes preparadas", fx:{rep:1}},
               {label:"Vou surpreender em breve", fx:{fans:1}} ] },
      { q:"As bolas paradas têm sido decisivas. Trabalho de bastidores?",
        opts:[ {label:"Treinamos isso todas as semanas", fx:{rep:1, fans:1}},
               {label:"Temos bons batedores", fx:{}},
               {label:"Sorte também conta", fx:{board:-1}} ] },
      { q:"Preferes controlar o jogo ou explorar o erro do adversário?",
        opts:[ {label:"Queremos a bola sempre", fx:{fans:2, board:-1}},
               {label:"Equilíbrio acima de tudo", fx:{board:1}},
               {label:"O que o jogo pedir", fx:{rep:1}} ] }
    ],
    // perguntas sobre um jogador pouco utilizado
    banco: [
      { q:"{jogador} quase não joga. Está fora dos teus planos?", target:"bench",
        opts:[ {label:"Conto com ele, a vez dele vem", fx:{pmorale:5, fans:1}},
               {label:"Tem de trabalhar mais para jogar", fx:{pmorale:-3, board:1}},
               {label:"Pode sair se aparecer proposta", fx:{pmorale:-6, fans:-1}, rumor:"{jogador} estaria de saída do {clube}."} ] },
      { q:"Os adeptos pedem para veres mais {jogador}. Dás-lhe minutos?", target:"bench",
        opts:[ {label:"Vai ter a sua oportunidade", fx:{pmorale:4, fans:3}},
               {label:"Joga quem merecer", fx:{board:1, fans:-1}},
               {label:"Não decido pela bancada", fx:{fans:-3, board:1}} ] },
      { q:"{jogador} mostrou-se descontente com a falta de jogo. Comentas?", target:"bench",
        opts:[ {label:"Falámos, está tudo resolvido", fx:{pmorale:3, morale:1}},
               {label:"Tem razões para estar chateado", fx:{pmorale:2, board:-1}},
               {label:"Quem quer jogar, treina para isso", fx:{pmorale:-4, board:1, morale:-1}} ] },
      { q:"Há quem diga que {jogador} é peso morto no plantel. Concordas?", target:"bench",
        opts:[ {label:"De todo, é importante para o grupo", fx:{pmorale:5, fans:1}},
               {label:"Está a ser avaliado", fx:{pmorale:-2}},
               {label:"O plantel é longo, é normal", fx:{}} ] },
      { q:"Vais dar titularidade a {jogador} para o valorizar no mercado?", target:"bench",
        opts:[ {label:"Não jogo com contas de mercado", fx:{rep:1, board:1}},
               {label:"Se render, joga e valoriza-se", fx:{pmorale:2}},
               {label:"Cada coisa a seu tempo", fx:{}} ] }
    ],
    ...
    misc: [
      ...
      { q:"Um adepto desafia-te: prometes bigode até ao fim da época se subires?",
        opts:[ {label:"Prometo o bigode!", fx:{morale:2, rep:1, fans:2}},
      ...
      { q:"Um miúdo pergunta-te se o {clube} vai ser campeão.",
        opts:[ {label:"Prometo-te que vamos tentar tudo!", fx:{fans:4, board:-1}},
               {label:"Vamos com os pés na terra", fx:{board:1}} ] },
      { q:"Desafiam-te a treinar de fato e gravata se ganhares a taça.",
        opts:[ {label:"Combinado, tenho palavra!", fx:{fans:3, rep:1}},
               {label:"Logo se vê", fx:{}} ] },
      { q:"Perguntam qual o teu prato preferido no pós-jogo.",
        opts:[ {label:"Bacalhau à moda cá da terra!", fx:{fans:2}},
               {label:"Só penso em futebol", fx:{}} ] },
